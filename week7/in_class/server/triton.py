...
class TritonClientSettings:
    ...
    def get_inference_client(self) -> Union[triton_http.InferenceServerClient, triton_grpc.InferenceServerClient]:
        """
        ...
        if self.parsed_url.scheme.startswith("http"):
            ssl_context_factory = (
                lambda: ssl.create_default_context(cafile=self.cert_path)  # pylint: disable=C3001
                if self.secure
                else None
            )
        ...
        raise ValueError(f"Invalid scheme {self.parsed_url.scheme}. Supported values: http, https, grpc.")

    def get_infer_input(self) -> Union[Type[triton_http.InferInput], Type[triton_grpc.InferInput]]:
        """
    ...
    def get_infer_requested_output(self) -> Union[Type[triton_http.InferRequestedOutput], Type[triton_grpc.InferRequestedOutput]]:
        """
...
class TritonClient:
    ...
    def _prepare_inputs(self, inputs: dict[str, np.ndarray]):
        infer_inputs = {k: self._infer_input(k, [len(v), v[0].shape[-1]], "INT64") for k, v in inputs.items()}
        for k, v in inputs.items():
            infer_inputs[k].set_data_from_numpy(v)
        return infer_inputs.values()

    def _get_outputs(self) -> Union[list[triton_http.InferRequestedOutput], list[triton_grpc.InferRequestedOutput]]:
    ...
    def predict(self, inputs: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        except (triton_http.InferenceServerException, triton_grpc.InferenceServerException) as e:
    ...
    def ping(self) -> tuple[Optional[str], bool]:  # pylint: disable=too-many-return-statements
        """
        ...
        if not self._client.is_server_live():
            return "FAILED : is_server_live", False
        if not self._client.is_server_ready():
            return "FAILED : is_server_ready", False
        if not self._client.is_model_ready(self.settings.model):
            return "FAILED : is_model_ready", False
        return None, True
